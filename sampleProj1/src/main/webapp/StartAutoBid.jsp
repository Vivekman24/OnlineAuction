	...
	if (result.next()) {
		...
		if (newAmount<=amountInAuction) {
			...
			if (result1.next()) {
				...
				if (newAmount>amountInAuction && oldAmount>newAmount) {
				...
				} else if (newAmount>amountInAuction && amountInAuction>=oldAmount && !currentlyWinning.equals(userName)) {
				...
				} else if (newAmount>oldAmount && oldAmount>=amountInAuction) {
			...
			while (result5.next()) {
				...
				ps3.setString(3, "The upper limit of your autobid" +" ($" + amount+ ") has been outbid for Auction " + auctionID + ". <a href='Autobid.jsp'>Manage your bids</a>.");
				ps3.executeUpdate();
			}
