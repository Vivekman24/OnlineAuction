...
<body> 	
 	...
 	<%
		...
		if(checker!=null && !checker.matches("true")){
			...
			if(!engine.isEmpty()){
				query += " AND middleDescription LIKE '%" + engine + "%'";
			}
			if(!transmission.isEmpty()){
				query += " AND longDescription LIKE '%" + transmission + "%'";
			}
		}
		...
		if (rs.next() == false) {
			...
			out.println("<th style='border: 1px solid #dddddd; text-align: left; padding: 8px;' onclick='sortTable(7)'>Engine Type</th>");
			out.println("<th style='border: 1px solid #dddddd; text-align: left; padding: 8px;' onclick='sortTable(8)'>Transmission</th>");
		...
		transmission = request.getParameter("transmission") != null ? request.getParameter("transmission") : "";
		
		out.println("<b>Active Filters:</b><br/>");
		if (!catID.isEmpty() || !subID.isEmpty() || !name.isEmpty() || !type.isEmpty() || !exactYear.isEmpty() || !newerYear.isEmpty() || !olderYear.isEmpty() || !color.isEmpty() || !engine.isEmpty() || !transmission.isEmpty()) {
	        out.println("<ul>");
	        ...
	        if (!engine.isEmpty()) out.println("<li>Engine Type: " + engine + "</li>");
	        if (!transmission.isEmpty()) out.println("<li>Transmission: " + transmission + "</li>");
	        out.println("</ul>");
	    } else {
		...
		out.println("<option value='VEHS'>Vehicle</option>");
		...
		out.println("<tr><td>Engine Type:</td><td><input type='text' name='engine'/></td></tr>");
		out.println("<tr><td>Transmission:</td><td><input type='text' name='transmission'/></td></tr>");
		...
		ResultSet rs1 = stmt.executeQuery("SELECT * FROM Wishlist w INNER JOIN Item i ON w.itemID = i.itemID WHERE userName ='" + userName + "'");
		if (!rs1.isBeforeFirst()) {
			...
			out.println("<th style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>Engine Type</th>");
			out.println("<th style='border: 1px solid #dddddd; text-align: left; padding: 8px;'>Transmission</th>");
		...
		function initializeDropper() {
			    ...
			    const categoryMap = {
			        "VEHS": [
			            ...
			            { value: "BIKS", text: "Bikes" },
			            { value: "PLNS", text: "Planes" },
			            { value: "TRKS", text: "Trucks" }
			        ]
			    };
