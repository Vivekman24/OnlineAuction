...
<body>
	...
	if (result.next()) {
		...
		if (newAmount1<=newAmount) {
			out.println("Upper limit should be greater than previous upper limit! <a href='Autobid.jsp'>Go back</a>.");
		} 
		else if (newAmount1<=amountInAuction) {
			...
			if (newAmount1>amountInAuction && oldAmount>newAmount1) {
				double newamount1WIncrement = newAmount1 + increment;
				if (newamount1WIncrement<=oldAmount) {
					String update = "UPDATE Auction SET currentWinner=?, currentPrice=? WHERE auctionID=?";
					PreparedStatement ps = con.prepareStatement(update);
					ps.setString(1, oldWinner);
					ps.setString(2, Double.toString(newamount1WIncrement));
					...
					ps1.setString(4, Double.toString(newamount1WIncrement));
					ps1.executeUpdate();
				}
			} 
			else if (newAmount1>amountInAuction && amountInAuction>=oldAmount && !currentlyWinning.equals(userName)) {
			...
			else if (newAmount1>oldAmount && oldAmount>=amountInAuction && !oldWinner.equals(userName)) {
			...
			String message = "Alert! Someone bid more than your upper limit for Auction " + auctionID+ ".";
			String delete = "DELETE FROM Alert WHERE endUserInfo='" + userName + "' and message='" + message+ "'";
			...
			while (result4.next()) {
				...
				ps3.setString(3, "Alert! Higher bid placed for Auction " + auctionID+ " for $" + curr_price+ ".");
				ps3.executeUpdate();
			}
			...
			while (result5.next()) {
				...
				ps3.setString(2, now.toString());
				ps3.setString(3, "Alert! Someone bid more than your upper limit for Auction " + auctionID+ ".");
				ps3.executeUpdate();
			}
